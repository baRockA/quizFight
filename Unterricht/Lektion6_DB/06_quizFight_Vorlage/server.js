...
const QuizDuel = require('./quiz-duel');
const RPSLSDuel = require('./rpsls-duel');
//Lektion 6 Aufgabe 2 c.) DataController einbinden
...
//Lektion 6 Aufgabe 2 c.) Variable für DataController Objekt deklarieren
...
function showReady() {
    ...
    //Lektion 6 Aufgabe 2 c.) datacontroller initialisieren

}

function connected(sock) {
    //Message Klasse verwenden und neues Objekt instanziieren
...
function onLogin(sock, data) {
    ...
    //Objekt der Player-Klasse instanziieren und zu lobby hinzufügen
...
function onChallenge(data) {
    ...
    let p1, p2 = null;
    //Instanz der Message Klasse erzeugen
    ...
    if (p1 != null && p2 != null) { // Wenn die Spieler noch in der lobby sind
        if (!chal) {
            ...
            switch (turn.game) {
                case 'rps':
                    newgame = new RPSDuel(p1, p2);
                    newgame.makeTurn(turn.player1, turn.turn);
                    break;
                case 'quiz':
                    //Lektion 6 Aufgabe 3 a.) datacontroller an QuizDuel übergeben
                    newgame = new QuizDuel(p1, p2);
                    break;
                case 'rpsls':
                    newgame = new RPSLSDuel(p1, p2);
                    newgame.makeTurn(turn.player1, turn.turn);
                    break;
                default:
                    throw new Exception('Spiel ist nicht bekannt.');
            }
            challenges.push({
            ...
            let isRunning = chal.game.makeTurn(turn.player1, turn.turn);
            if (!isRunning) {
